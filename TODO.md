...
- [x] ペット手帳: ペット写真アイコンにコスチューム・デコ・スタンプが反映されていない → 免許証完成画像から選択して設定可能にした（2026-03-18）
- [x] ペット手帳: ペット追加時にアイコン画像を設定できない → ActionSheet（カメラロール/免許証から選ぶ）を実装（2026-03-18）
...
### 実装済み（実機テストで確認すること）
- [ ] エディタ: 4モードタブ切替（スワイプ+タップ）が正常に動作するか
- [ ] エディタ: ブラシオフセット方向が正しいか（左上→左上にカーソル）→ unitOffset符号を反転済み
- [ ] エディタ: オフセットのロングプレス放射メニューが動作するか
- [ ] エディタ: 消しゴム再タップでOFFになるか
- [ ] エディタ: ガイドボタンでブラシツール使用中でもON/OFFできるか
- [ ] エディタ: 色調整スライダー（明るさ/コントラスト/彩度）が動作するか
- [ ] タグ注文: 丸型プレビューの金色枠線が消えているか + 位置ずれが直っているか
- [ ] ペット手帳: 写真設定タップでActionSheet（カメラロール/免許証から選ぶ）が表示されるか
- [ ] ペット手帳: 免許証から選んだ画像がアイコンに反映されるか
- [ ] コレクション: デフォルトでペット毎グループ表示になっているか + セクション見出しが表示されるか
