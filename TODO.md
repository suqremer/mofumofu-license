...
- [x] ペット手帳: ペット写真アイコンにコスチューム・デコ・スタンプが反映されていない → 免許証完成画像から選択して設定可能にした（2026-03-18）※ただし不具合あり→3/18一覧参照
- [x] ペット手帳: ペット追加時にアイコン画像を設定できない → ActionSheet（カメラロール/免許証から選ぶ）を実装（2026-03-18）※ただし不具合あり→3/18一覧参照
- [ ] 設定画面: プレミアム購入後に「無料枠を使い切りました」が即時消えない → 実機で既にプレミアム購入済みのため確認不可。別Sandboxアカウントで確認する
- [x] 免許証交付完了画面: 「シェアする」→ 動作確認OK（2026-03-18）
- [x] フレーム&デコ: フレーム色・証明写真背景色などの項目を、選択したら展開するアコーディオン式UI → 確認OK（2026-03-18）
- [x] フレーム&デコ: コスチューム写真調整の項目を一番上に移動 → 確認OK（2026-03-18）
- [x] 投げ縄/ブラシ: タッチ位置が指で隠れて見えない → ブラシオフセット機能実装（4方向+放射メニュー）、方向反転修正済み（2026-03-18）
- [x] コレクション: 作成済み免許証を押したら複製する機能 → 確認OK（2026-03-18）
- [x] 投げ縄/消しゴム/復元: ツール選択中はガイド表示を非表示にする → ガイドボタンで手動ON/OFFに変更、ツール使用中でもトグル可能（2026-03-18）
- [x] 作成済み免許証の編集: 証明写真以外の要素が表示されない → 確認OK（2026-03-18）
- [ ] 実物注文: 複数免許証の同時注文に対応 → 完成イメージを左右スクロールで複数枚表示

## 実機テスト修正一覧（2026-03-18）

### 実装済み確認結果
- [x] エディタ: 4モードタブ切替（スワイプ+タップ）→ OK
- [x] エディタ: ブラシオフセット方向が正しいか → OK
- [x] エディタ: オフセットのロングプレス放射メニュー → OK
- [x] エディタ: 消しゴム再タップでOFF → OK
- [x] エディタ: ガイドボタンでブラシツール使用中でもON/OFF → OK
- [x] エディタ: 色調整スライダー（明るさ/コントラスト/彩度）→ OK
- [ ] タグ注文: 丸型プレビューの金色枠線+位置ずれ → 未確認
- [x] コレクション: デフォルトでペット毎グループ表示 → OK（ただしUI改善要望あり→下記）

### 不具合・改善要望（要対応）
- [x] ペット手帳: 免許証から選んだ画像がアイコンに反映されない → 証明写真部分をクロップしてPNG保存するように修正（2026-03-18）
- [x] ペット手帳: 編集画面の写真アイコンがペット切り抜きのみ → クロップ済み画像で自動解決（2026-03-18）
- [x] ペット手帳: 「免許証から選ぶ」のサムネイルが免許証中心部分 → PhotoCropPreviewで証明写真クロップ表示に変更（2026-03-18）
- [x] コレクション: ペット毎グループ表示のUI改善 → 2列ペットカード+タップでシームレス画面遷移して免許証一覧表示（2026-03-18）
- [x] エディタ: ガイドシステムをモード別に変更 → コスチューム:顔ガイド線+GIF / 背景削除:GIF4種 / デコ・色調整:チュートリアルなし（2026-03-18）GIF素材5本完成済み
- [x] エディタ: ガイドの顔シルエット線はコスチュームタブのみ表示 → _mode==EditorMode.outfitで条件分岐（2026-03-18）

### GIF素材（2026-03-18 完了）
- [x] assets/tutorial/guide_costume.gif — コスチュームタブ: 顔合わせ+コスチューム選択
- [x] assets/tutorial/guide_brush_auto.gif — 背景削除: 自動背景削除
- [x] assets/tutorial/guide_brush_offset.gif — 背景削除: オフセット操作
- [x] assets/tutorial/guide_brush_lasso.gif — 背景削除: 投げ縄操作
- [x] assets/tutorial/guide_brush_finish.gif — 背景削除: 消しゴム+復元で仕上げ
※ 構成変更: デコタブのチュートリアル不要に。背景削除は自動削除→オフセット→投げ縄→仕上げの4ステップ

### 追加修正（2026-03-18 後半）
- [x] デコ: 画面どこでも2本指ピンチで拡大縮小可能に（Positioned.fill方式、インスタストーリー風）
- [x] デコ: ピンチ中のハンチング修正（2本指中は移動無効、拡大縮小+回転のみ）
- [x] デコ・色調整: 写真本体が動かないよう制限（移動はコスチュームタブのみ）
- [x] コレクション: アイコンをPhotoCropPreviewに変更（証明写真クロップ表示）
- [x] コレクション: ペットタップでシームレス画面遷移（Fade+SlideUp）して免許証一覧表示
- [x] ペット手帳 編集: 免許証があればコスチューム反映済みアイコン表示
- [x] ペット手帳: クロップ座標をテンプレートoutputSizeから実画像サイズに修正
- [x] ペット手帳: 写真変更後のリスト反映修正（pet.photoPath優先表示）
- [ ] ペット手帳: 免許証から選んだ画像の表示確認（クロップ座標修正後、実機テスト必要）
- [ ] ペット手帳 編集: 写真変更→更新後のリスト反映確認（実機テスト必要）
