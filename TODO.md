...
| 3 | 有料アプリ契約（銀行口座+納税フォーム） | しゅーと | 納税フォーム提出済、口座処理中（24-48h待ち）→完了後に有効化 |
| 4 | サポート用メールアドレス取得 | しゅーと | Done |
| 5 | Firebase Crashlytics導入 | Claude | Done |
| 6 | 写真権限拒否時の挙動確認・実装 | Claude | Done |
| 7 | 小規模事業者プログラム申請 | しゅーと | 待ち（有料アプリ契約が有効になってから申請） |
...
| 34 | 年齢レーティング回答 | しゅーと | |
| 35 | App Privacy（栄養ラベル）申告 | しゅーと+Claude | Claude側Done（docs/app_privacy.md、7データ種別+入力手順+ATT説明。しゅーとがApp Store Connectで入力） |
