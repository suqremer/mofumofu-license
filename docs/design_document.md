...
NDEF メッセージに **2つのレコード** を書き込む。URIレコードを必ず1番目に配置すること（iOSのバックグラウンドタグ読み取りは1番目のレコードしか見ないため）。
...
特記: [note]（任意）
```
- 既存の `readTag` 処理がテキストレコードを読み取っているため後方互換のため残す
- アプリ内のNFC読み取り機能はこちらを使う

### 9.2 制約
- NTAG215: 最大504バイト（実効ペイロード約480バイト）
- リアルタイムバイトカウンター表示
- 書き込みタイムアウト: 30秒
- 2レコード合計のNDEFメッセージサイズは `estimateNdefMessageBytes()` で事前チェック
...
- v1.0.5: NFC URIレコード対応（iPhoneでアプリ不要で読み取り可能）+ 写真パスバグ修正（PathResolverセルフヒーリング、DBバージョン4マイグレーション、preview_screen保存時相対化）
