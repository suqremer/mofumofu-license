...
```
①注文内容を組み立てる（アプリ）
   免許証選択 / タグ・セットは丸形画像作成 / NFC代行チェック / 備考
   送信前にサムネ確認「この写真で作ります」
...
- **NFC代行の連絡先**: 決済画面の任意入力は機能不全のため不採用。当面しゅーとが +¥500 請求メールで内容確認（アプリは `nfcProxy` フラグのみ送る）。頻度が増えたら +¥500 別SKU化を検討。
...
- **リリース順序**: Android Closed Testing 14日完走→製品版申請の**後**に新フローを投入（審査スケジュール衝突回避）。iOS/Android同時には出さず段階公開。
