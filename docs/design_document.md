...
| 免許証テンプレート（番号） | モノスペース | 大文字スペーシング |
| 免許証テキスト（Android） | IBM Plex Sans JP | 400/700 ※iOSはヒラギノ（未指定） |

#### 免許証描画のフォントとOS別文字位置補正（設計判断）
`license_painter` の Canvas 描画で、(1)**Android は iOS よりフォントのベースライン位置が高く（文字が上にずれて）見える**メトリクス差、(2)**OSデフォルト漢字フォントが iOS=ヒラギノ角ゴ / Android=Noto Sans CJK で異なり Android の見た目が iOS と離れる**問題があった。
- **フォント方針（2026-06-22 更新）**: iOS は **`fontFamily` 未指定＝ヒラギノを維持**（既存ユーザーの見た目を変えない）。**Android のみ免許証テキストの `fontFamily` を `IBM Plex Sans JP` に指定**し iOS のヒラギノに寄せた（実機で BIZ UDPGothic / Zen Kaku Gothic New / IBM Plex Sans JP を比較して IBM Plex を採用）。フォントは `pubspec.yaml` の `fonts:` でバンドル（`main.dart` で `allowRuntimeFetching=false` のためDLは使わない）。
  - ※旧方針（〜2026-06-16）は「fontFamily明示は採らずY座標補正のみ」だったが、iOSを触らずAndroidだけ指定する形で**部分的にfontFamily明示へ変更**。
- **位置補正方針**: `Platform.isAndroid` 限定で各テキストの Y 座標を調整（iOS は v1.1.0 当時のまま不変＝iOS が正）。フォントをIBM Plexにしたことで2026-06-22に補正量を再調整。
- 補正対象（2026-06-22時点）: 氏名(ラベル/値)・住所・交付・有効期間・品種・生年月日・優良バッジ・番号・二小原/他/二種・ハンコ「ウチノ子」・生年月日ひみつ「ひ・み・つ」。補正量は実機で確認した実測値。
- 縦書きハンコ・ひみつ表記は1文字ずつ描画するため、**漢字/カタカナ/中点だけをピンポイントで補正**できる。
...
- **本命＝起動時検知**: アプリ起動時に端末ローカルの pending 注文（写真未送信）を検知し、ホーム最上部にバナーを出す。ディープリンクが不発でも**アプリを開き直すだけで復帰できる**最も確実な層。
  - **未送信バナーの状態管理（557で一元管理に刷新）**: 未送信バナーは `pendingOrdersProvider`（FutureProvider、`database_provider.dart`）を `home_screen` が `ref.watch` して表示する。**注文作成（`order_card/tag_screen` の `upsertOrder` 後）・写真送信完了（`order_upload_screen` の uploaded 更新後）・削除（同 deleteOrder 後）のたびに `ref.invalidate(pendingOrdersProvider)` を呼び、どの画面から操作してもバナーが自動で最新になる**（`order_upload_screen` は `ConsumerStatefulWidget`）。注文の入口が4箇所（ホームのバナー/ギャラリー・コレクション・プレビュー完成画面）に分散しているため、画面ごとの再取得ではなく provider の一元管理で担保した。※経緯：初期（556）は削除後の戻りを `context.canPop() ? context.pop() : context.go('/')` にして再取得を発火させる方式だったが、注文作成後にホームへ直接戻るとバナーがすぐ出ない穴が残ったため、557で provider 一元管理に置換した。
