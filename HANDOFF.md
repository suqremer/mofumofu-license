...
## 最終作業日
2026-06-25（**v1.1.3 リリース作業＋未送信バナー不具合の修正**）＝①前回の続きでホーム画面UI改善（バナーのキャッチ文を全幅1行化＝「うちの子を、手に取れるグッズに。」の字余り2行を解消／FTUE初回起動の「作ってみる」サンプルを実物の免許証画像 `assets/tutorial/sample_license.png` に差し替え）→ iOS TestFlight・Android実機で確認OK → **v1.1.3+555**（コミット `a4a880e`）。②**バグ修正**＝未送信注文を削除してもホームの「写真未送信」バナーが残る不具合（画面遷移で戻ると消える）。原因＝`order_upload_screen` の削除後の戻りが `context.go('/')` で、ホーム `home_screen._resumePendingOrder` の `await push` 戻り後の `_pendingOrdersFuture` 再取得を発火させなかったこと。修正＝戻りを `context.canPop() ? context.pop() : context.go('/')` に変更（push経由なら pop で戻り→再取得→バナー即消える）。**v1.1.3+556**・コミット `c9fba4b`・push済み。⚠️**Android実機での最終確認は帰宅後**（出先でUSB接続不可）。③**バグ修正②の実機確認OK**（Android実機で kDevMode を一時ONにして枚数制限を外し、削除→未送信バナーが即消えることを確認。確認後 kDevMode=false に戻し済み）。④**未送信バナーを一元管理に刷新（案C・v1.1.3+557）**＝556の削除修正だけでは「注文後にホームへ直接（スワイプ）戻ると未送信バナーがすぐ出ない（他タブ経由なら出る）」問題が残った。原因＝ホームの未送信チェックが `initState`（起動時）と `_resumePendingOrder`（バナー経由遷移の戻り）でしか走らず、注文の入口が4箇所（ホームのバナー`home_screen:373`／ギャラリー`:730`・コレクション`collection_screen:407`・プレビュー完成画面`preview_screen:867`）に分散していたため。**根本解決＝`pendingOrdersProvider`（FutureProvider）を新設し、注文作成・写真送信完了・削除のたびに `ref.invalidate` でホームのバナー（`ref.watch`）を自動更新**。`order_upload_screen` を `ConsumerStatefulWidget` 化。変更＝`database_provider.dart`/`home_screen.dart`/`order_card_screen.dart`/`order_tag_screen.dart`/`order_upload_screen.dart`。`flutter analyze` 今回分クリーン（残13 issuesは全て既存の別ファイルのinfo）。**Android実機で全経路（ホーム/コレクション/プレビューからの注文→即バナー・削除→即消え・送信完了→消え）確認OK**。コミット `42ebc1b`・push済み。⑤**リリース状況（v1.1.3+557）＝両OS審査提出完了**。Android：ローカルAAB 557→**Closed Test 557 審査提出済み**。iOS：Codemagic 557→**App Store 557 審査提出済み**（変更はDartロジック＝プラットフォーム非依存＋Android実機確認済みのため TestFlight再確認は省略）。iOS提出素材＝**スクショ `Desktop/skusyo_ios/1-8.png`（1284×2778へリサイズ＝App Store の 6.7"要件。元 `Desktop/skusyo` は1290×2796・6.9"で温存）**・説明文 `docs/aso_text.md`・リリースノート（ホーム改善・グッズ導線・サンプル表示・表示更新不具合修正）。**次の宿題＝両OS審査通過→iOS公開（手動リリース）・Android Closed Test 14日完走→製品版申請**。⚠️据え置き＝旧方式（アプリ側コード/Remote Config/旧Googleフォーム）撤去（両OS製品版後）・App Check Enforce切替＋NFC実機再確認・SafetyNet依存解消（次バージョン）・Stripe PayPay追加（将来検討）。<br>
2026-06-22（**ホーム画面の実物グッズ導線を改善**＝①バナーを `set_overview.jpg`（カード＋タグが両方写った写真）入りに刷新。文言「うちの子を、手に取れるグッズに。／本格カード＆タグ／¥2,280〜 お家にお届け」＋CTA「作ってみる ›」、クレジットカードのアイコンは廃止（課金と誤解されるため）。②「うちの子グッズ」写真ギャラリーをタップで `/order` へ（`ProductGallery`に`onTap`追加・見出しに「タップで詳しく ›」）。変更=`home_screen.dart`/`product_gallery.dart`。`flutter analyze` クリーン。⚠️**出先でUSB/Wi-Fi接続できずバナーのレイアウトを未実機確認＝次回ビルドできる時にバナー写真の高さ(現状150)や上下の切れ方を実機で必ず微調整すること**。UX/UIチーム検討の結論に基づく実装）<br>
