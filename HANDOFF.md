...
## 最終作業日
2026-04-10（v1.0.6 ヘルプ機能追加・審査中 / Instagram アクションブロック解除待ち）
...
| 6 v1.0.6アップデート | 🔄 審査中 | ヘルプ・よくある質問機能追加（2026-04-10 提出済み、TestFlight実機動作確認OK） |
| マーケ施策 | 🔄 実行中 | minne審査中、Creema公開済み、Instagram `@uchinoko_co` 開設済み（アクションブロック解除待ち） |

## 直近セッションでの変更（2026-04-10）

### v1.0.6 ヘルプ・よくある質問機能追加（commit 6ffd60e, 00d067a, push済み）
- **新規ファイル**:
...
| # | タスク | 参照ドキュメント | 備考 |
|---|--------|----------------|------|
| 1 | **v1.0.6 審査通過待ち** | — | 通常1〜2日。リリース時はApp Store Connect手動でリリース（変更内容は軽微なので低リスク） |
| 2 | **Instagram アクションブロック解除待ち** | 上記「直近セッション」参照 | 2026-04-11以降にPCブラウザで `@uchinoko_co` のプロフィールが見られるか確認→アプリに戻る。**24〜72時間放置必須、何も操作しない** |
| 3 | **解除後: 広告用3枚セット再投稿** | キャプション案は上記参照 | [丸形ペットA][card_display.jpg正方形][丸形ペットB]。**右→中央→左の順、1枚ずつ間隔あけて投稿** |
| 4 | **広告用投稿の中央タイルをブースト広告化** | — | フィード配置のみ、¥100/日×30日=¥3,000、CTA「アプリをダウンロード」 |
| 5 | minne 3商品の審査通過待ち | — | 通常1〜2日 |
| 6 | TikTokアカウント開設＋動画投稿 | `docs/marketing_plan.md` セクション3 | 動画案10本あり。特に#2,6,10がおすすめ |
| 7 | SNSで「Creemaに出品しました」告知 | — | Twitter/Xで開発ストーリーと共に |
| 8 | App Storeスクリーンショット改善 | `docs/marketing_plan.md` セクション4.5 | 犬猫の魅力的な作例を追加 |
| 9 | 実機で広告表示確認 | — | 本番アプリで広告枠に実広告が出るかチェック（数日は在庫不足の可能性あり） |
| 10 | Firebase Analyticsで`ad_impression`イベント確認 | — | Firebase Console→Analytics→イベントで`ad_impression`を確認 |
