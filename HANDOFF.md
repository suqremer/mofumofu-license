...
## 最終作業日
2026-04-07（minne/Creema出品完了 + NFC URI実装 + 写真パスバグ修正 + NFC容量対策 + v1.0.5取り下げ→再ビルド予定）
...
| 5 v1.0.5アップデート | 🔄 再ビルド準備中 | NFC URI対応 + 写真パスバグ修正 + NFC容量対策（審査取り下げ済み、再submit予定） |
...
| # | タスク | 参照ドキュメント | 備考 |
|---|--------|----------------|------|
| 1 | Codemagic で v1.0.5 を再ビルド | — | 取り下げ済み。NFC容量対策込みで再submit |
| 2 | TestFlight v1.0.5の動作確認 | — | NFC書き込み（容量問題解消確認）、iPhoneでかざして自動通知、写真パス修正が効くか、特記事項50文字制限 |
| 3 | App Store Connectで v1.0.5 を再submit | — | テスト通過後 |
...
| 8 | AdMob app-ads.txt認証待ち | — | `docs/app-ads.txt` 設置済み、GitHub Pages公開済み |
